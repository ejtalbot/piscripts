...
def lengthen_sequence(sequence: List[str], multiplier: int):
    return [
        sub_item
        for sub_items in [[item] * multiplier for item in sequence]
...
def create_color_pattern_by_name(color_names: List[str]) -> List[Tuple[str, str, str]]:
    ...
    color_name_dict = read_to_color_name_dict(
        os.path.join(
            os.path.abspath(os.path.dirname("resources/colors.csv")), "colors.csv"
        )
    )
...
def modulo_position_in_count(position: int, count: int):
    return position if position < count else (position + 1) % count
